...
-- stylua: ignore
for _, lhs in ipairs {
  'C-q', 'C-o', 'C-a', 'C-h', 'C-j', 'C-k', 'C-l', 'C-z', 'C-v', 'C-b', 'C-_', 'C-/', 'S-Del',
...
require('lazy').setup({
  ...
  --
  -- Show you all of the keymaps for the current Telescope picker:
  --  - Insert mode: <c-/>
  --  - Normal mode: ?
  --
  ...
  { 'Bilal2453/luvit-meta', lazy = true },
  {
    ...
    config = function()
      ...
      --
      -- If you're wondering about lsp vs treesitter, you can check out `:help lsp-vs-treesitter`
    ...
    config = function()
      ...
      cmp.setup {
        ...
        mapping = cmp.mapping.preset.insert {
          ...
          -- https://github.com/hrsh7th/nvim-cmp/issues/1038#issuecomment-2526189226
          ['<C-CR>'] = function()
            ...
            local sl = selected_entry.replace_range['start'].line
            local sc = selected_entry.replace_range['start'].character
            local cursor = vim.api.nvim_win_get_cursor(win)
            local el = cursor[1] - 1
            local ec = cursor[2]

            vim.api.nvim_buf_set_text(buf, sl, sc, el, ec, { selected_entry.word })
            vim.api.nvim_win_set_cursor(win, { el + 1, sc + #selected_entry.word })
