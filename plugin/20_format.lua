...
pack.later(function()
  ...
  local format_buf = function(buf, opts)
    buf = buf or 0
  ...
  local format = function(local_opts)
    ...
    if not vim.is_callable(conf) then
      if local_opts.or_lsp then
        ...
        silent_call(local_opts.silent, function()
  ...
  vim.api.nvim_create_user_command("AutoformatToggle", function()
    ...
    vim.notify(string.format("Autoformat toggle: %s", vim.g.format_on_save))
  end, {})
