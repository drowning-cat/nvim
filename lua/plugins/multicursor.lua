...
return {
  {
    ...
    config = function()
      ...
      map({'n','v'}, '<C-N>', function() mc.matchAddCursor(-1) end, { desc = 'Add a [N]ew cursor above by matching' })
      map({'n','v'}, '<C-n>', function() mc.matchAddCursor(1) end, { desc = 'Add a [n]ew cursor below by matching' })
      map({'n','v'}, '<C-x>', function() mc.deleteCursor() end, { desc = 'Remove the main cursor' })
