...
local favorite = require 'plugins.snacks.misc.favorite'
favorite.setup {
  letters = '1234567890',
  set_key = function(letter)
...
table.insert(plugins, {
  ...
  keys = {
    ...
    { '\\', function() Snacks.picker.explorer { cwd = vim.u.find_root() } end, desc = 'Toggle explorer' },
    { '<leader>se', function() Snacks.picker.explorer { cwd = vim.u.find_root(), layout = 'floating_sidebar', auto_close = true } end, desc = '[S]earch [e]xplorer' },
    ...
    { '<leader><space>', function() favorite.view.favorite_picker() end, desc = 'Move to favorite' },
    { '<leader>+', function() favorite.api.add_buf() end, desc = 'Add to favorite' },
  },
})
