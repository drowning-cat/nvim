...
local function picker_system(picker, cmd_name)
...
function M.pick_pulls(opts)
  return Snacks.picker.pick(vim.tbl_deep_extend('keep', opts or {}, {
    ...
    confirm = function(picker, item)
      picker_system(picker)({ 'gh', 'pr', 'checkout', item.pr_number }, { timeout = 10000 }, function(out)
        picker:close()
        vim.schedule(function()
          Snacks.notify[out.code == 0 and 'info' or 'error'](out.stderr)
        end)
      end)
    end,
